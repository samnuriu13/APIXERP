    ...
    public class CommandBuilder
    {
        ...
        public void GetInsertCommand(IList<ItemBase> insertedList)
        {
            ...
            foreach (ItemBase item in insertedList)
            {
                ...
                foreach (ItemStorage field in item.Fields)
                {
                    if (!field.FieldType.Equals(FieldCategory.PrimaryField) &&
                        !field.FieldType.Equals(FieldCategory.NormalField)) continue;
                    insertColumns.AppendFormat("{0},", field.Name);
                    insertValue.AppendFormat("{0},", field.GetValue());
                }
        ...
        public void GetUpdateCommand(IList<ItemBase> updatedList)
        {
            ...
            foreach (ItemBase item in updatedList)
            {
                ...
                List<ItemStorage> updatedCols = item.Fields.FindAll(col => col.Modified);
                if (updatedCols.Count.Equals(0)) continue;

                foreach (ItemStorage field in item.Fields)
                {
                    if (field.Modified && field.FieldType.Equals(FieldCategory.NormalField))
                        updatedColumn.AppendFormat("{0} = {1},", field.Name, field.GetValue());

                    if (field.FieldType.Equals(FieldCategory.PrimaryField) ||
                        field.FieldType.Equals(FieldCategory.PrimaryIdentityField))
                        whereValue.AppendFormat(" {0} = {1} And", field.Name, field.GetValue());
                }
                //
