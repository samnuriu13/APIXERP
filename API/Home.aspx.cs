...
namespace API
{
    public partial class Home : PageBase
    {
        ...
        protected void lnkPersonalContent_Click(Object sender, EventArgs e)
        {
            try
            {
                Server.Transfer("~/UI/EmployeeBasicInfo/EmployeeBasicInformation.aspx?empcode=" + ltlEmpCode.Text + "&EmpKey=" + CurrentUserSession.EmpKey + "&selectedtab=" + 9);
            }
            catch (Exception ex)
            {
        ...
        protected void lnkLeave_Click(Object sender, EventArgs e)
        {
            try
            {
                Server.Transfer("~/UI/LeaveManagement/LeaveTransaction.aspx?empCodeForLeave=" + ltlEmpCode.Text + "&EmpKey=" + CurrentUserSession.EmpKey + "&Flag=1");
            }
            catch (Exception ex)
            {
