...
namespace API.UI.Security
{
    public partial class GroupInformation : PageBase
    {
        ...
        private CustomList<SECURITY.DAO.SecurityRule> SecurityRuleList
        {
            get
            {
                if (Session["SecurityRule_SecurityRuleCodeList"] == null)
                    return new CustomList<SECURITY.DAO.SecurityRule>();
                else
                    return (CustomList<SECURITY.DAO.SecurityRule>)Session["SecurityRule_SecurityRuleCodeList"];
            }
            set
            {
        ...
        //private CustomList<SECURITY.DAO.Menu> MenuList
        //{
        //    get
        //    {
        //        if (Session["SecurityRule_MenuList"] == null)
        //            return new CustomList<SECURITY.DAO.Menu>();
        //        else
        //            return (CustomList<SECURITY.DAO.Menu>)Session["SecurityRule_MenuList"];
        //    }
        //    set
        //    {
        ...
        //    try
        //    {
        //        //MenuList = new CustomList<SECURITY.DAO.Menu>();
        //        //MenuList = manager.GetAllMenuByApplicationID(drpApplication.SelectedValue.ToString());
        //        CustomList<SECURITY.DAO.SecurityRule> SelectedAllSecurityRuleList = SecurityRuleList.FindAll(f => f.IsSaved);
        //        CustomList<RuleDetails> sROList = new CustomList<RuleDetails>();
        //        if (SelectedAllSecurityRuleList.IsNotNull())
        //        {
        //            foreach (SECURITY.DAO.SecurityRule sR in SelectedAllSecurityRuleList)
        //            {
        ...
        protected void btnFind_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                StaticInfo.SearchItem(items, "Group Info", "SearchGroup", FRAMEWORK.SearchColumnConfig.GetColumnConfig(typeof(Group), columns), 500);
                EnableAllControls(true);
            }
            catch (Exception ex)
            {
